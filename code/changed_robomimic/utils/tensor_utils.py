...
from .torch_to_tf import torch_tensor_clone, torch_unsqueeze, torch_from_numpy, torch_tensor_float, torch_zeros

from .torch_to_tf import torch_tensor_byte, torch_max, torch_mean, torch_sum, torch_ones_like, torch_repeat_interleave, torch_cat
...
def clone(x):
    ...
    return recursive_dict_list_tuple_apply(
        x,
        {
            ...
            tf.Tensor: lambda x: torch_tensor_clone(x),
...
def unsqueeze(x, dim):
    ...
    return recursive_dict_list_tuple_apply(
        x,
        {
            ...
            tf.Tensor: lambda x: torch_unsqueeze(x, dim=dim),
...
def to_tensor(x):
    ...
    return recursive_dict_list_tuple_apply(
        x,
        {
            ...
            np.ndarray: lambda x: torch_from_numpy(x),
            type(None): lambda x: x,
        }
    )
...
def to_float(x):
    ...
    return recursive_dict_list_tuple_apply(
        x,
        {
            ...
            tf.Tensor: lambda x: torch_tensor_float(x),
...
def to_uint8(x):
    ...
    return recursive_dict_list_tuple_apply(
        x,
        {
            ...
            tf.Tensor: lambda x: torch_tensor_byte(x),
...
def to_one_hot_single(tensor, num_class):
    ...
    assert 0 == 1, "to_one_hot_single is not implemented right now"
    # x = torch.zeros(tensor.size() + (num_class,)).to(tensor.device)
    x = torch_zeros(tensor.size() + (num_class,))
...
def flatten(x, begin_axis=1):
    ...
    return recursive_dict_list_tuple_apply(
        x,
        {
            ...
            tf.Tensor: lambda x, b=begin_axis: flatten_single(x, begin_axis=b),
        }
    )
...
def reshape_dimensions(x, begin_axis, end_axis, target_dims):
    ...
    return recursive_dict_list_tuple_apply(
        x,
        {
            ...
            tf.Tensor: lambda x, b=begin_axis, e=end_axis, t=target_dims: reshape_dimensions_single(
...
def join_dimensions(x, begin_axis, end_axis):
    ...
    return recursive_dict_list_tuple_apply(
        x,
        {
            ...
            tf.Tensor: lambda x, b=begin_axis, e=end_axis: reshape_dimensions_single(
...
def named_reduce_single(x, reduction, dim):
    ...
    if reduction == "flatten":
        ...
        x = torch_max(x, dim=dim)[0]  # [B, D]
    elif reduction == "sum":
        x = torch_sum(x, dim=dim)
    else:
        x = torch_mean(x, dim=dim)
    return x
...
def pad_sequence_single(seq, padding, batched=False, pad_same=True, pad_values=None):
    ...
    assert isinstance(seq, (np.ndarray, tf.Tensor))
    ...
    repeat_func = np.repeat if isinstance(seq, np.ndarray) else torch_repeat_interleave
    concat_func = np.concatenate if isinstance(seq, np.ndarray) else torch_cat
    ones_like_func = np.ones_like if isinstance(seq, np.ndarray) else torch_ones_like
...
def pad_sequence(seq, padding, batched=False, pad_same=True, pad_values=None):
    ...
    return recursive_dict_list_tuple_apply(
        seq,
        {
            ...
            tf.Tensor: lambda x, p=padding, b=batched, ps=pad_same, pv=pad_values:
